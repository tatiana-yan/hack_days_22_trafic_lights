...
def getLight(i, green, red):
    interval = (i * 10) % ((green+red) * 10)
    print(interval)
    limit = 0.95
    if interval < green*10:
        return 1 if np.random.rand() < limit else 0 #1
    else:
        return 0 if np.random.rand() < limit else 1

def get_data_4():
    x = range(60*100)#get_x(60*100)
    n = len(x)
    y = [0.5] * n
    w = [1] * n
    for i in range(n):
        if i < 15 * 100:
            y[i] = 1
        if i >= 15*100 and i < 20*100:
            y[i] = getLight(i, 100, 100)
        if i >= 20*100 and i < 40*100:
            y[i] = getLight(i, 150, 60)
        if i >= 40*100 and i < 45*100:
            y[i] = getLight(i, 100, 100)
        if i >= 45*100 and i < 60*100:
            y[i] = getLight(i, 150, 60)
...
def main():
    ...
    knots = list[int](range(0, 60*100, 31))
#    knots = [0, 1500, 2000, 4000, 4500, 6000]
    knots.append(points.x[len(points) - 1])

    
    s = splpckg.Spline(coefficients, knots, 2)#3)
    c = CurveFitter(s)
    ...
    plt.plot(x_curve, y_curve, points.x, points.y, 'o', knots, knots_y_uni, 's', points.x, [0.5]*len(points))
    plt.legend(["Spline", "Data", "Knots", "toggle"])
